...
| Urgent health concern | Stop the beta session, redirect to qualified support outside the app, avoid diagnosis or coaching, and record only product-safety metadata. | Session blocked; product issue severity is Critical if app copy contributed. |

High reports that do not create immediate harm still require a private issue, assigned owner, target fix/review date, and explicit decision before the next beta cohort.
...
- Retain tester text only as long as needed for the beta triage decision, then remove unnecessary prose from private notes.
