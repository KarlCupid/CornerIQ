...
4. Fix: make targeted fixes only for reviewed launch blockers or harness blockers. Do not add broad product features during a QA pass.
...
Page-text snapshots should use the active surface scope when a `testID` exists. Today snapshots use the Today surface, Fuel uses the active Fuel section, Train uses the active Train section, Plan uses the active Plan section, and Profile Safety/Data use their active Profile sections. If the harness falls back to full `document.body`, the snapshot manifest must label that as a `document.body fallback`.

## Max Iterations

Use at most two fix iterations per Codex pass unless the user explicitly asks for more:

- Iteration 1: repair harness failures or obvious launch blockers found by automation.
- Iteration 2: verify the fix and address one additional tightly scoped blocker if it is clearly related.

After two iterations, distill the remaining findings into `QA_LOOP_STATE.md` and stop with a clear next action. This avoids infinite loops and keeps release decisions reviewable.
...
## Launch-Ready Meaning

`launch_code_ready` means:

- All automatable launch-readiness gates pass.
- No Blocker findings remain.
- No High findings remain.
- No Medium finding marked `must fix before launch` remains.
- Human-only gates are explicitly listed as `human_review_required`, `accepted_launch_limitation`, or complete with real evidence.
...
`external_launch_ready` additionally requires release-owner evidence for the distribution path, accepted build artifact, app metadata, private distribution control, and build-config secret safety.
...
Human review is a first-class output of the loop. When automation reaches its boundary, the next action should name the exact human session needed, such as `physical iPhone review`, `live Supabase/release-owner check`, `AI qualitative review`, or `human boxer validation`.
