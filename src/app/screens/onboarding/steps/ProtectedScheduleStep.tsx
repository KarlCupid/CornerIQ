...
import {
  effortForExistingTrainingIntensity,
...
export function ProtectedScheduleStep({ draft, setStepError, updateDraft }: OnboardingStepProps) {
  ...
  const saveWorkout = () => {
    ...
    const next: RecurringProtectedWorkoutAnchorDraft = {
      ...
      weekday,
      durationMinutes,
      intensity: intensityForExistingTrainingEffort(rpe),
  ...
  const editWorkout = (index: number) => {
    ...
    setRpe(effortForExistingTrainingIntensity(workout.intensity));
    ...
    <View style={{ gap: spacing.lg }}>
      ...
      <FieldGroup helper="Choose every part included in this workout." label="Workout includes">
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
          {existingTrainingComponentOptions.map((option) => <ChipButton active={components.includes(option.value)} key={option.value} label={option.label} onPress={() => toggleComponent(option.value)} />)}
        </View>
      </FieldGroup>
      ...
      {components.length > 1 ? (
        <FieldGroup helper="Choose the part that takes the most work, or leave it balanced." label="Main part">
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
            ...
            {components.map((component) => <ChipButton active={primaryComponent === component} key={component} label={existingTrainingComponentOptions.find((option) => option.value === component)?.label ?? component} onPress={() => setPrimaryComponent(component)} />)}
          </View>
        </FieldGroup>
      ) : null}

      {components.includes("boxing") ? <FieldGroup label="Boxing work"><View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>{existingBoxingOptions.map((option) => <ChipButton active={boxingFormat === option.value} description={option.description} key={option.value} label={option.label} onPress={() => setBoxingFormat(option.value)} />)}</View></FieldGroup> : null}
      {components.includes("strength") ? <FieldGroup helper="What area does the strength work mainly train?" label="Strength area"><View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>{existingStrengthOptions.map((option) => <ChipButton active={strengthArea === option.value} description={option.description} key={option.value} label={option.label} onPress={() => setStrengthArea(option.value)} />)}</View></FieldGroup> : null}
      {components.includes("conditioning") ? <FieldGroup label="Conditioning format"><View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>{existingConditioningOptions.map((option) => <ChipButton active={conditioningFormat === option.value} description={option.description} key={option.value} label={option.label} onPress={() => setConditioningFormat(option.value)} />)}</View></FieldGroup> : null}
      ...
      <FieldGroup helper="1 is very easy. 10 is an all-out effort." label="Expected effort"><View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>{existingTrainingEffortOptions.map((value) => <ChipButton active={rpe === value} key={value} label={String(value)} onPress={() => setRpe(value)} />)}</View></FieldGroup>
