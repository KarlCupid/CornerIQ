...
function expectActiveWorkspaceBeforeOverview(output: string, focusedTestId: string): void {
  ...
  const overviewIndex = output.indexOf("plan-visual-dashboard");
...
describe("minimal app screens", () => {
  ...
  it("AppTabs orders daily tabs and applies Today route intents once", async () => {
    ...
    expect(output).toContain("Today's fuel");
  });
  ...
  it("FuelScreen renders the start-here action path before raw details", async () => {
    ...
    expect(output.indexOf("Do now")).toBeLessThan(output.indexOf("Today's fuel"));

    await act(async () => {
      await press(pressableWithText(renderer, "Show fuel detail"));
    });
    output = JSON.stringify(renderer.toJSON());
    expect(output).toContain("Today's fuel");
  ...
  it("FuelScreen renders safety review up front without dangerous instructions", async () => {
    ...
    expect(output.indexOf("Safety stop")).toBeLessThan(output.indexOf("Today's fuel"));
    expect(output).toContain("Safety stop");
  ...
  it("TrainScreen does not show future materialized sessions early but loads them on their date", async () => {
    ...
    expect(planOutput).toContain("This week");
  });
  ...
  it("PlanScreen renders warnings", async () => {
    ...
    await switchSection(warningRenderer, "Diagnostics");
    warningOutput = JSON.stringify(warningRenderer.toJSON());
  ...
  it("PlanScreen moves selected workflows into the active workspace", async () => {
    ...
    await switchSection(scheduleRenderer, "Edit fixed schedule");
  ...
  it("Plan and Train agree on generated support count, dates, and titles", async () => {
    ...
    expect(planOutput).toContain("Days:");
  });
  ...
  it("PlanScreen lets athletes add, edit, and remove fixed boxing sessions", async () => {
    ...
    expect(JSON.stringify(renderer.toJSON())).toContain("Edit fixed schedule");
    await act(async () => {
      await press(pressableWithText(renderer, "Edit fixed schedule"));
    });
  ...
  it("ProfileScreen renders privacy notes", async () => {
    ...
    expect(output).toContain("Safety stops require");
  ...
  it("fatigue-first screens keep collapsed sections and primary actions short", async () => {
    ...
    expect(fuelShowButtons).toHaveLength(1);
    expect(fuelShowButtons[0]).toContain("Show fuel detail");
    expect(JSON.stringify(fuelRenderer.toJSON())).toContain("Today's fuel");
    ...
    expect(JSON.stringify(planRenderer.toJSON())).toContain("This week");
