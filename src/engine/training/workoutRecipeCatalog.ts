...
function rest(title: string, exerciseId: string | undefined, doThis = "Breathe. Shake out your shoulders. Reset your stance before the next round."): WorkoutRecipeStep {
  return step({
    type: "rest",
    title,
    durationSeconds: 60,
...
function boxingRecipe(input: {
  ...
  rounds: readonly Omit<WorkoutRecipeStep, "stepId" | "type" | "durationSeconds" | "autoAdvance" | "exerciseId">[];
  ...
  const roundSteps = input.rounds.flatMap((round, index) => {
    ...
    return index < input.rounds.length - 1 ? [roundStep, rest(`Rest ${index + 1}`, input.mainExerciseId)] : [roundStep];
  });
  const blocks = [
    input.warmup,
    block({
...
function jabFocusedRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    previewFlow: [
      ...
      "Boxing rounds - low and slow shadow, sharp jab round, jab entry and exit, best clean jab round",
    ...
    rounds: [
      ...
      {
        title: "Round 2: Sharp jab focused round",
      ...
      {
        title: "Round 3: Jab entry and exit",
      ...
      {
        title: "Round 4: Best clean jab round",
...
function entryExitRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    mainExerciseId: mainId,
    cooldown: boxingCooldown(cooldownId),
    previewFlow: ["Warm-up - stance bounce and jab shape", "Boxing rounds - jab in, double jab entry, clean exits", "Cooldown - breathing and shakeout"],
    ...
    rounds: [
      ...
      { title: "Round 4: Best entry and exit", doThis: "Use the entry and exit that stayed cleanest. Repeat it without adding extra work.", coachCue: "Clean entry. Clean exit.", microCues: ["Hands home.", "Exit first.", "Finish sharp."] }
    ]
  });
}
...
function defenseResetRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    mainExerciseId: mainId,
    cooldown: boxingCooldown(cooldownId),
    previewFlow: ["Warm-up - shoulders, stance, easy shadow flow", "Boxing rounds - jab plus slip, jab-cross plus roll, balanced reset", "Cooldown - breathing and easy range"],
    ...
    rounds: [
      ...
      { title: "Round 4: Best defense reset", doThis: "Use the defensive move that stayed balanced. Remove anything messy.", coachCue: "Balanced defense only.", microCues: ["Feet under you.", "Hands home.", "Do not admire the work."] }
    ]
  });
}
...
function footworkAnglesRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    mainExerciseId: mainId,
    cooldown: boxingCooldown(cooldownId),
    previewFlow: ["Warm-up - stance bounce and step reset", "Boxing rounds - quiet feet, jab then angle, clean exits", "Cooldown - breathing and shakeout"],
    ...
    rounds: [
      ...
      { title: "Round 2: Jab then small angle", doThis: "Touch a jab, then step to a small angle and freeze in stance.", coachCue: "Touch first. Step second.", microCues: ["Small angle.", "Guard home.", "Feet under you."] },
      { title: "Round 3: Exit before adding more", doThis: "Punch once or twice, exit, and reset before adding another action.", coachCue: "Leave before you chase.", microCues: ["Exit clean.", "Do not spin.", "Stay balanced."] },
      { title: "Round 4: Best angle round", doThis: "Repeat the angle that stayed clean. Keep it simple.", coachCue: "Best angle, not most movement.", microCues: ["Quiet feet.", "Hands home.", "Finish sharp."] }
    ]
  });
}
...
function counterTimingRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    mainExerciseId: mainId,
    cooldown: boxingCooldown(cooldownId),
    previewFlow: ["Warm-up - stance and light rhythm", "Boxing rounds - feint, pause, answer, reset", "Cooldown - breathing and easy range"],
    ...
    rounds: [
      ...
      { title: "Round 4: Best timing round", doThis: "Repeat the timing pattern that stayed calm and clean.", coachCue: "Calm timing beats fast guessing.", microCues: ["Wait.", "Touch.", "Reset."] }
    ]
  });
}
...
function bagRecipe(input: WorkoutRecipeResolutionInput, variant: "jab" | "combo"): WorkoutRecipe {
  ...
  if (variant === "jab") {
    return boxingRecipe({
      ...
      previewFlow: ["Warm-up - bag distance and light jab touch", "Bag rounds - light jab, sharp jab, jab and exit", "Cooldown - breathing and shakeout"],
      ...
      rounds: [
        ...
        { title: "Round 2: Sharp jab touch", doThis: "Add a little snap to the jab, but keep the hand coming back clean.", coachCue: "Snap without reaching.", microCues: ["Hand back.", "Chin quiet.", "Stay on balance."] },
        { title: "Round 3: Jab and exit", doThis: "Jab the bag, step out, and reset before touching again.", coachCue: "Exit after the touch.", microCues: ["Step small.", "Guard home.", "Do not admire the shot."] },
        { title: "Round 4: Best clean bag jab", doThis: "Use your cleanest jab touch. Keep power capped so technique stays sharp.", coachCue: "Clean beats hard.", microCues: ["Hands home.", "Feet under you.", "Finish sharp."] }
      ]
    });
  }
  return boxingRecipe({
    ...
    cooldown: boxingCooldown(cooldownId),
    blockTitle: "Bag rounds",
    previewFlow: ["Warm-up - bag distance and easy rhythm", "Bag rounds - jab-cross reset, jab-cross exit, body-head shape", "Cooldown - breathing and shakeout"],
    ...
    rounds: [
      ...
      { title: "Round 4: Best short combination", doThis: "Use the short combo that stayed clean. Keep it sharp and controlled.", coachCue: "Best combo, not most punches.", microCues: ["Clean finish.", "Guard home.", "Exit before more."] }
    ]
  });
}
...
function conditioningRecipe(input: WorkoutRecipeResolutionInput): WorkoutRecipe {
  ...
  return boxingRecipe({
    ...
    rounds: [
      ...
      { title: "Round 4: Coordination cap", doThis: "Stop adding pace if coordination or breathing changes.", coachCue: "Finish with control left.", microCues: ["Clean beats tired.", "Feet under you.", "Leave one gear unused."], safetyStop: EASY_STOP }
    ]
  });
}
